...
def new():
    if request.method == "POST":
        ...
        return redirect(url_for("process", name=name, days=days, exp=level))
...
@app.route("/program_<name>_<days>days_<exp>")
def process(name, days, exp):    
    return render_template("program_done.html", name=name, days=days, exp=exp)
